...
**Ray 在真实业务里沉淀的 AI Skill 工具箱——不给「该怎么想」的建议，直接把「下一步」干掉**

VPS 开荒 · 节点巡检 · thread 装配 · 数据周报 · 对标拆解 · 企业诊断 · 方案蓝图 · 产品概念 · 站点上线 · 周复盘
...
一个给 AI Agent（Claude Code / Codex 等）用的 builder 工具箱。每个 skill 背后是一段**真实发生的实践**：跑着付费客户的 API 中转、上过线的 B2B 站、交付过的咨询案、处置过的入侵事故、日更的 X 内容。
...
| 一段刚发生的实战 / 事故 | 装配成 build-in-public thread 骨架，观点留白给你填 | `/ray-thread` |
...
| 线 | 命令 | 干什么 | 背后的账本 |
|---|---|---|---|
| 🧭 路由 | **`/ray`** | 交任务，自动分发 | 单步动态导航 |
| 🛠 基建 | `/ray-vpsinit` | VPS 开荒：加固 + 代理栈 + 交接 | proxy-fleet 实战 |
| | `/ray-nodecheck` | 节点 / 中转链健康巡检（只读） | dmit 探针 |
| ✍️ 内容 | `/ray-thread` | 实战 → thread 骨架（不代笔） | build-in-public |
| | `/ray-tweet` | 每日社会观察 / 哲理推文候选 | X 内容管线 |
| | `/ray-metrics` | X 账号数据周报，找传播规律 | KOL 复盘 |
| | `/ray-benchmark` | 对标拆解，判可迁移性 | 拆竞品的复盘 |
| | `/ray-report` | magazine 长文（HTML + PDF + 公众号） | 编辑设计语言 |
| 🔍 咨询 | `/ray-diagnose` | 企业 AI 就绪度诊断（六维 + 红黄绿） | 青屿咨询案 |
| | `/ray-proposal` 🔒 | 诊断结论 → 方案蓝图 | groundwork 方法论 |
| 📦 产品 | `/ray-idea` | 从批判框架锻造产品概念 | 铁砧 |
| | `/ray-launch` | 落地页 / B2B 站上线全流程 | fohosocks 上线 |
| 🧹 内务 | `/ray-weekly` 🔒 | 每周复盘：项目 + 数据 + 现金流 | 内务自动化 |
| | `/ray-cleanup` 🔒 | 归档沉睡项目、磁盘瘦身 | 归档纪律 |
...
<div align="center">
<sub>rayskills · 一套姓 ray 的实战工具箱 · 作者 <a href="https://x.com/imraywang">@imraywang</a></sub>
</div>
