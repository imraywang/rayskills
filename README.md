...
- **一致优先**:即使某个 skill 有独立产品潜力,也留在 `/ray-*` 命名里——花名册整齐 > 个别 skill 的独立身份。(anvil 曾按"作品"单列,现已并入为 `/ray-anvil`。)
...
### 📦 产品 Product
- **`/ray-anvil`** — 铁砧:从消费社会批判框架锻造经得起拆的产品概念。
...
本仓库是所有成员 skill 的唯一真源。本地 `~/.claude/skills/` 与 `~/.agents/skills/` 一律软链到本仓,禁止独立副本。若某个 skill 的逻辑被内嵌进别处(如 web 产品的 prompt),该派生须声明血缘并注明基于的版本。
