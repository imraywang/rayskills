---
name: ray-x-article
description: 把已经完成的中文长文和 5:2 封面可靠写入登录中的 X Articles 后台，保存为可继续编辑的草稿，检查自动保存与预览，并把草稿地址回写到 rays-brain。用于“把这篇文章放进 X 后台”“保存为 X Article 草稿”“接上 X 长文发布管线”或检查、续写已有 X Article 草稿时。不得自动点击 Publish，不得用普通 Post 接口冒充 Article，也不得重复创建同题草稿。
---
...
3. X 没有公开的 Article 草稿接口。使用 `chrome:control-chrome` 操作用户已登录的后台，不使用普通 Post API 或 `xurl post`。
...
脚本负责去掉 frontmatter、生成纯文本与富文本 HTML、检查标题与正文、确认长文有二级标题和重点加粗、拒绝连续空行、确认封面接近 5:2，并输出编辑器应有的块数、标题数、加粗数、首尾锚点与指纹。检查失败就退回 `ray-writer` 或 `ray-cover` 修正，不绕过。

## 2. 进入 X Articles

先完整阅读并使用 `chrome:control-chrome`。再阅读 [x-editor-contract.md](references/x-editor-contract.md)，按照已验证的可见文字定位编辑器。
...
5. 等待页面出现 `Last saved just now` 或等价的已保存状态。

Chrome 拒绝读取本地封面时，不要继续尝试其他上传手段，也不要创建新草稿。记录 `draft-needs-cover` 和当前 URL，请用户开启扩展的本地文件访问权限；收到确认后从原 URL 续传。
